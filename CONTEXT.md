...
- Session start: modal confirms **$100** starting credit. OK awards `credit`, shows **Credit: $N** and **Bet: $N** top-left, reveals the face-down deck pile, and shows bet buttons.
...
- No Hit/Stand, scoring, or payout yet.
...
- Each card gets an `AtlasTexture` from a tight grid: `width / 13` by `height / 4`. The sheet is generated by `tools/generate_face_sheet.py` (**120×223** cells, same as `CardUI.CARD_SIZE`, no gutters).
...
- Display size is `120×223` (`CARD_SIZE`), 1:1 with one face-sheet cell so faces stay sharp.
...
- After the deal: naturals, Hit/Stand, dealer rule, payout.
...
- Keep Blackjack rules out of menu scenes. Deal, scoring, and dealer AI belong under `scripts/`.
- Do not add a second `deck_manager.gd` in the project root.
- Dealer AI may use only the dealer hand and the hit-below-17 rule.
- Next sensible step: score the opening hands (soft Ace, natural), then Hit/Stand.
